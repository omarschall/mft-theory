...
def equation_2_14(Delta_0, g=1.2, n_hermite=200):
    ...
    return Delta_0**2 / 2 - g**2 * (gaussian_integral(squared_Phi_sqrt_, n_hermite=n_hermite)
                                    - gaussian_integral(Phi_sqrt_, n_hermite=n_hermite) ** 2)

def equation_2_14_prime(Delta_0, g=1.2, n_hermite=200):
    ...
    GI1 = gaussian_integral(squared_Phi_sqrt_prime_, n_hermite=n_hermite)
    GI2 =  gaussian_integral(Phi_sqrt_, n_hermite=n_hermite)
    GI3 = gaussian_integral(Phi_sqrt_prime_, n_hermite=n_hermite)

    return Delta_0 - g**2 * (GI1 - 2 * GI2 * GI3)

### --- Numerically solve for Delta_0 --- ###

def solve_for_Delta_0(g, Delta_0_init=2, max_iters=1000, tol=1e-6, n_hermite=200):
    """For a given value of g and initial guess for Delta_0, use Newton's method
    to solve for Delta_0."""

    Delta_0 = Delta_0_init
    y = equation_2_14(Delta_0, g=g, n_hermite=n_hermite)
    N_iters = 0
    while np.abs(y - 0) > tol and N_iters < max_iters:
        y = equation_2_14(Delta_0, g=g,n_hermite=n_hermite)
        Delta_0 = np.maximum(Delta_0 - y / equation_2_14_prime(Delta_0, g=g, n_hermite=n_hermite),
                             0.000001) #keep Delta_0 positive
        N_iters += 1

    return Delta_0

def phi_autocorrelation(Delta, Delta_0,n_hermite=200):
...
def solve_for_Delta_T(g, Delta_0, T=10, dT=0.01, n_hermite=200):
    ...
    Delta_T[0] = Delta_0
    C_Phi_T = np.zeros_like(time_vector)
    C_Phi_T[0] = phi_autocorrelation(Delta_0, Delta_0, n_hermite=n_hermite)
    for i_t, t in enumerate(time_vector):
        ...
        Delta_T[i_t] = np.maximum(Delta_T[i_t], 0) #keep Delta_T positive
        C_Phi_T[i_t] = phi_autocorrelation(Delta_T[i_t], Delta_0, n_hermite=n_hermite)
        Delta_T_dotdot = Delta_T[i_t] - g**2 * C_Phi_T[i_t]
        Delta_T_dot[i_t] = Delta_T_dot[i_t-1] + dT * Delta_T_dotdot

    return time_vector, Delta_T, Delta_T_dot, C_Phi_T

def compute_alpha(Delta_0):
    phi_prime = lambda z, Delta_0: 1 - np.tanh(np.sqrt(Delta_0) * z)**2
    return gaussian_integral(partial(phi_prime, Delta_0=Delta_0), n_hermite=200)

def Delta_potential(g, Delta, Delta_0, n_hermite=200):
