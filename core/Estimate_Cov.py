...
def estimate_cov_eigs(T_sim, dt_save, dt, W, phi_torch=phi_torch,
    ...
    for i_loop in range(N_loops):
        ...
        x_xcovs[i_loop] = x_xcov
        r_xcovs[i_loop] = r_xcov
    x_xcov_mean = x_xcovs.mean(dim=(0, 1))
    r_xcov_mean = r_xcovs.mean(dim=(0, 1))

    eigs_x, vecs = torch.linalg.eigh(x_xcov_mean)
    eigs_r, vecs = torch.linalg.eigh(r_xcov_mean)
